...
func (s *Server) AuthHandler(providerName, rule string) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// CHANGE: Parse the X-Forwarded-For header to get the real client IP
		var ipAddr string
		forwardedFor := escapeNewlines(r.Header.Get("X-Forwarded-For"))
		if forwardedFor != "" {
...
func (s *Server) logger(r *http.Request, handler, rule, msg string) *logrus.Entry {
	// CHANGE: Parse the X-Forwarded-For header to log the real client IP
	var sourceIp string
	forwardedFor := escapeNewlines(r.Header.Get("X-Forwarded-For"))
	if forwardedFor != "" {
		ips := strings.Split(forwardedFor, ",")
